...
<body style="background-color: #099FFC;">
	...
	<header style="background-color:white;">
		<h1 align="center" style = "text:blue;text-decoration: underline;">Gerenciador de Tarefas (To-do List)</h1>
	</header>
				...
				<form method = "post" action="insere_tarefa.asp" id = "formulario_insercao">
					...
					<div class="mb-3">
						...
						<div class="input-group">
						  <input type="text" class="form-control" name="tituloDaTarefa" id="tituloDaTarefa" placeholder="Título da tarefa" required>
						</div>
					</div>
					...
					<div class="mb-3">
						...
						<div class="input-group">
					 	 <textarea class="form-control" name="descricaoDaTarefa" id="descricaoDaTarefa"  placeholder="Descrição da tarefa" form="formulario_insercao" required></textarea>
						</div>
					</div>
					...
					<div class="mb-3">
						...
						<div class="input-group">
						  <input type="date" class="form-control" onFocus = "desabilitarDatasPassadas()" name="dataDeVencimentoDaTarefa" id="dataDeVencimentoDaTarefa" required>
						</div>
					</div>
					<div class="d-flex justify-content-between">
					      <div></div>
					      <div class="ml-auto">
					         <input class="btn btn-primary"  type="submit" name="submit" value = "Adicionar tarefa">
					      </div>
					 </div>
				</form>
			</div>
			...
			If Err.Number <> 0 Then
				...
				if Err.Number <> 0 then
					...
					If recordSet.BOF And recordSet.EOF Then
					    Response.write "<p>Não possui tarefas</p>"
					Else
						  			...
						  			Response.write "<div class='col-sm-6'>"
							  			Response.write "<div class='detalhamentoTarefa'>"
											Response.write "<h5>" & recordSet(1) &"</h5>"
						  					Response.write "<div style = ""overflow:auto; height:110px;"">"
						    				Response.write recordSet(2)
						    				Response.write "</div>"
						    				Response.write "<div style= ""margin-top:10px;""> Vencimento: " & FormatDateTime(recordSet(3),2) & "</div>"
						    				Response.write "<button type=""button"" class=""btn btn-danger btn-sm"" onclick=""removeTarefa('" & recordSet(0) & "')"">Excluir</button>"
						    				Response.write "<button type=""button"" class=""btn btn-secondary btn-sm"" onclick=""editarTarefa('" & recordSet(0) & "','" & recordSet(1) & "','" & recordSet(2) & "','" & recordSet(3) & "')"">Editar</button>"
			    						Response.write "</div>"
			    					Response.write "</div>"
						    recordSet.MoveNext
						Loop
